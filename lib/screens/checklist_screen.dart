...
class _ChecklistScreenState extends State<ChecklistScreen> {
  ...
  ItemModel? _temporaryVisibleItem; // ❗ 필터링되어있어도 검색으로 선택 시 임시로 보여줄 항목
        ...
        builder: (context, setModalState) => AlertDialog(
                ...
                children: [
                  ...
                  if (col != 'itemCode') ...[
                    ...
                    if (col == 'remarks') ...[
                      ...
                      TextField(controller: includeController),
                      ...
                      const Text("수량 직접 입력", style: TextStyle(fontWeight: FontWeight.bold)),
                      TextField(controller: quantityController, keyboardType: TextInputType.number),
          ...
          actions: [
            ...
            TextButton(onPressed: () {
              setState(() {
                _isSorted = localIsSorted; if (localIsSorted) { _currentSortCol = col; _isAscending = localIsAscending; } else if (_currentSortCol == col) _currentSortCol = "";
                if (_columnFilters.containsKey(col)) _columnFilters[col] = localFilters;
                if (col == 'remarks') { _remarksFilterQuery = includeController.text; _remarksExcludeQuery = excludeController.text; _remarksIncludeLogic = localIncludeLogic; _remarksExcludeLogic = localExcludeLogic; }
                if (col == 'quantity') _quantitySearchQuery = quantityController.text;
              });
              _applyFilterAndSort(); Navigator.pop(ctx);
    ...
    showDialog(context: context, builder: (ctx) => DefaultTabController(length: 2, child: StatefulBuilder(builder: (context, setDialogState) => AlertDialog(
      ...
      content: SizedBox(width: double.maxFinite, height: 450, child: TabBarView(children: [
        SingleChildScrollView(child: Column(children: [
          const SizedBox(height: 20), TextField(controller: ipController, decoration: const InputDecoration(labelText: "IP 주소", border: OutlineInputBorder())),
          const SizedBox(height: 10), TextField(controller: userController, decoration: const InputDecoration(labelText: "ID", border: OutlineInputBorder())),
          const SizedBox(height: 10), TextField(controller: passController, obscureText: obscurePass, decoration: InputDecoration(labelText: "PW", border: const OutlineInputBorder(), suffixIcon: IconButton(icon: Icon(obscurePass ? Icons.visibility : Icons.visibility_off), onPressed: () => setDialogState(() => obscurePass = !obscurePass)))),
          const SizedBox(height: 20), ElevatedButton.icon(onPressed: () async {
            String? err = await _smbService.testConnection(ipController.text, userController.text, passController.text);
            if (err == null) { List<String> shares = await _smbService.listShares(); _showError("성공", "✅ 접속 성공!\n\n[공유 목록]\n${shares.join('\n')}"); }
            else _showError("오류", "접속 실패: $err");
        ...
        Column(children: [
          Expanded(child: ReorderableListView(onReorder: (o, n) { setDialogState(() { if (n > o) n -= 1; final String item = _processList.removeAt(o); _processList.insert(n, item); }); }, children: [
            for (int i = 0; i < _processList.length; i++) ListTile(key: ValueKey(_processList[i] + i.toString()), title: Text(_processList[i]), trailing: const Icon(Icons.drag_handle), leading: IconButton(icon: const Icon(Icons.remove_circle_outline, color: Colors.red), onPressed: () => setDialogState(() => _processList.removeAt(i)))),
          ])),
          const Divider(),
          Row(children: [Expanded(child: TextField(controller: newProcessController, decoration: const InputDecoration(hintText: "공정명 추가"))), IconButton(icon: const Icon(Icons.add_box, color: Colors.green, size: 35), onPressed: () { if (newProcessController.text.isNotEmpty) setDialogState(() { _processList.add(newProcessController.text); newProcessController.clear(); }); })]),
        ]),
      ])),
      actions: [
        TextButton(onPressed: () => Navigator.pop(ctx), child: const Text("취소")),
        TextButton(onPressed: () async {
          await prefs.setString('smbIp', ipController.text); await prefs.setString('smbUser', userController.text); await prefs.setString('smbPass', passController.text); await prefs.setStringList('processList', _processList);
          _smbService.setConfig(ipController.text, userController.text, passController.text); setState(() {}); Navigator.pop(ctx);
  ...
  void _showSmbFiles(String share, String path, String mode) async {
    ...
    List<Map<String, dynamic>> filteredFiles = files.where((f) {
      if (f['isDirectory'] as bool) return true;
      String name = (f['name'] as String).toLowerCase();
      return mode == 'file' ? (name.endsWith('.xlsx') || name.endsWith('.xls')) : name.endsWith('.pdf');
    }).toList();
    showDialog(context: context, builder: (ctx) => AlertDialog(title: Text("$share/$path"), content: SizedBox(width: double.maxFinite, height: 400, child: Column(children: [
      ...
      Expanded(child: ListView.builder(itemCount: filteredFiles.length, itemBuilder: (c, i) {
        final f = filteredFiles[i]; bool isDir = f['isDirectory'] as bool; String name = f['name'] as String;
        return ListTile(leading: Icon(isDir ? Icons.folder : Icons.description), title: Text(name), onTap: () { if (isDir) { Navigator.pop(ctx); _showSmbFiles(share, "${path == "" ? "" : "$path/"}$name", mode); } else if (mode == 'file') { Navigator.pop(ctx); _downloadAndLoad(share, "${path == "" ? "" : "$path/"}$name"); } });
      })),
    ])), actions: [if (mode == 'dir') TextButton(onPressed: () { setState(() => _pdfFolderPath = "smb://$share/$path"); _saveSettings(); Navigator.pop(ctx); }, child: const Text("현재 폴더 선택")), TextButton(onPressed: () => Navigator.pop(ctx), child: const Text("취소"))]));
  }
  ...
  Future<void> _downloadAndLoad(String share, String remotePath) async {
    ...
    setState(() => _isLoading = false);
    if (file != null) _loadExcelData(file.path); else _showError("오류", "다운로드 실패");
  }
  ...
  Future<void> _syncAllPdfs() async {
    ...
    try {
      ...
      _showSnackBar("✅ ${targets.length}개 동기화 완료!");
  ...
  void _showFileBrowser(String mode, String initialPath) {
    showDialog(context: context, builder: (ctx) => StatefulBuilder(builder: (context, setModalState) {
      final dir = Directory(initialPath); List<FileSystemEntity> entities = [];
      try { entities = dir.listSync().where((e) { if (e is Directory) return true; return mode == 'file' ? (e.path.endsWith('.xlsx') || e.path.endsWith('.xls')) : e.path.endsWith('.pdf'); }).toList(); entities.sort((a, b) => p.basename(a.path).toLowerCase().compareTo(p.basename(b.path).toLowerCase())); } catch (_) {}
      return AlertDialog(title: Text(p.basename(initialPath)), content: SizedBox(width: double.maxFinite, height: 400, child: Column(children: [
      ...
      ])), actions: [if (mode == 'dir') TextButton(onPressed: () { setState(() => _pdfFolderPath = initialPath); _saveSettings(); Navigator.pop(ctx); }, child: const Text("현재 폴더 선택")), TextButton(onPressed: () => Navigator.pop(ctx), child: const Text("취소"))]);
    }));
  }
  ...
  void _handleRefresh() {
    _forgetFocus(); if (_excelPath.isEmpty) return;
    if (File(_excelPath).existsSync()) { _loadExcelData(_excelPath); _showSnackBar("🔄 리스트를 새로고침했습니다."); }
  }

  void _showComplementDialog(ItemModel item) {
    _forgetFocus(); showDialog(context: context, builder: (ctx) => AlertDialog(title: const Text("보완 선택"), content: Column(mainAxisSize: MainAxisSize.min, children: [
  ...
  Widget _buildSummaryWidget(bool isDark) {
    ...
    int total = dataItems.length; int completed = dataItems.where((i) => i.complete).length;
    final fItems = _displayItems.where((i) => !i.isSubheading && i.realIndex != -1);
    int fTotal = fItems.length; int fComp = fItems.where((i) => i.complete).length;
    bool isFiltered = total != fTotal || _showUnfinishedOnly;
    return InkWell(onTap: () { setState(() => _showUnfinishedOnly = !_showUnfinishedOnly); _applyFilterAndSort(); }, child: Container(padding: const EdgeInsets.symmetric(horizontal: 8), child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: [
      Text("[전체 $total / 완료 $completed / ${(total>0?(completed/total*100):0).toStringAsFixed(1)}%]", style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: isDark ? Colors.white70 : Colors.blueGrey[800])),
      if (isFiltered) Text("[필터 $fTotal / 완료 $fComp / ${(fTotal>0?(fComp/fTotal*100):0).toStringAsFixed(1)}%]", style: const TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: Colors.orangeAccent)),
    ])));
  }
  ...
  Future<void> _handleItemClick(ItemModel item) async {
    _forgetFocus(); if (_autoSave) _manualSave(silent: true);
    if (_pdfFolderPath.startsWith("smb://")) {
      ...
      try {
        ...
        await _smbService.downloadFile(share, remoteFilePath, "$_baseDownloadPath/CheckSheet/${item.itemCode}.pdf");
    ...
    final String? lastItemCode = await Navigator.push<String>(context, MaterialPageRoute(builder: (_) => PdfViewerScreen(
      allItems: _originalItems.where((i) => !i.isSubheading).toList(), // ❗ 전체 데이터 (검색용)
      filteredItems: _displayItems.where((i) => !i.isSubheading && i.realIndex != -1).toList(), // ❗ 필터된 데이터 (이동용)
    ...
    if (lastItemCode != null) {
      ...
      if (!isDisplayed) {
        ...
        setState(() { _temporaryVisibleItem = targetItem; }); _applyFilterAndSort();
      }
  ...
  Widget _headerBtn(String label, String? colKey, double? width) {
    ...
    return InkWell(onTap: colKey == null ? null : () => _sortBy(colKey), child: Container(width: width, alignment: Alignment.center, child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [Flexible(child: Text(dLabel, style: TextStyle(color: (isNoFilt || isFiltActive) ? Colors.yellow : Colors.white, fontWeight: FontWeight.bold, fontSize: 13), overflow: TextOverflow.ellipsis)), if (isTarget) Icon(_isAscending ? Icons.arrow_drop_up : Icons.arrow_drop_down, color: Colors.yellow, size: 18)])));
  }
  ...
  Widget _buildDataRow(ItemModel item, bool isDark) {
    ...
    return GestureDetector(onTap: () { if (_isEditMode) setState(() { if (isSel) _selectedIndices.remove(item.realIndex); else _selectedIndices.add(item.realIndex); }); }, child: Container(
      ...
      child: Row(children: [
        ...
        Expanded(flex: 5, child: InkWell(onTap: () => _handleItemClick(item), child: Container(padding: const EdgeInsets.symmetric(horizontal: 8), child: FittedBox(fit: BoxFit.scaleDown, alignment: Alignment.centerLeft, child: Text(item.itemCode, style: TextStyle(fontSize: 13, color: isDark ? Colors.blue[300] : Colors.blue[700], fontWeight: FontWeight.bold)))))),
        ...
        _cellText(item.process, Colors.blueGrey, isDark, () => _showProcessDialog(item)),
